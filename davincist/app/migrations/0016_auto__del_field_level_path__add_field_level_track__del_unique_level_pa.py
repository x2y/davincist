...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.delete_unique('app_level', ['path_id', 'name'])
        db.delete_unique('app_level', ['path_id', 'rank'])

        db.rename_column('app_level', 'path_id', 'track_id')

        db.create_unique('app_level', ['track_id', 'rank'])
        db.create_unique('app_level', ['track_id', 'name'])


    def backwards(self, orm):
        # User chose to not deal with backwards NULL issues for 'Level.path'
        raise RuntimeError("Cannot reverse this migration. 'Level.path' and its values cannot be restored.")
