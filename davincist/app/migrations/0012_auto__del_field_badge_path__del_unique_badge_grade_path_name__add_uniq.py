...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Badge', fields ['grade', 'path', 'name']
        db.delete_unique('app_badge', ['grade', 'path_id', 'name'])

        # Deleting field 'Badge.path'
        db.delete_column('app_badge', 'path_id')
    ...
    def backwards(self, orm):
        ...
        # User chose to not deal with backwards NULL issues for 'Badge.path'
        raise RuntimeError("Cannot reverse this migration. 'Badge.path' and its values cannot be restored.")
        # Adding unique constraint on 'Badge', fields ['grade', 'path', 'name']
        db.create_unique('app_badge', ['grade', 'path_id', 'name'])


    models = {
        'app.badge': {
            'Meta': {'ordering': "['level', 'name', 'grade']", 'unique_together': "(('name', 'grade', 'level'),)", 'object_name': 'Badge'},
