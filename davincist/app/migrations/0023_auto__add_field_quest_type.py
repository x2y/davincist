...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Quest.type'
        db.add_column('app_quest', 'type',
                      self.gf('django.db.models.fields.CharField')(default='H', max_length=1),
                      keep_default=False)
        if not db.dry_run:
          for quest in orm.Quest.objects.all():
            if quest.is_senior_validated:
              quest.type = 'S'
            elif quest.is_peer_validated:
              quest.type = 'P'
            else:
              quest.type = 'H'
            quest.save()
    ...
    def backwards(self, orm):
        ...
        db.delete_column('app_quest', 'type')
