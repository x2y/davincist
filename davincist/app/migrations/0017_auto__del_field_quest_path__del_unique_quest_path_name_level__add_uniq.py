...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Quest', fields ['path', 'name', 'level']
        db.delete_unique('app_quest', ['path_id', 'name', 'level_id'])

        # Deleting field 'Quest.path'
        db.delete_column('app_quest', 'path_id')

        # Adding unique constraint on 'Quest', fields ['name', 'level']
        db.create_unique('app_quest', ['name', 'level_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Quest', fields ['name', 'level']
        db.delete_unique('app_quest', ['name', 'level_id'])


        # User chose to not deal with backwards NULL issues for 'Quest.path'
        raise RuntimeError("Cannot reverse this migration. 'Quest.path' and its values cannot be restored.")
        # Adding unique constraint on 'Quest', fields ['path', 'name', 'level']
        db.create_unique('app_quest', ['path_id', 'name', 'level_id'])
